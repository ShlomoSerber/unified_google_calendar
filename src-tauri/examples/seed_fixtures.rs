...
fn dt(monday: NaiveDate, day: i64, h: u32, m: u32) -> String {
    ...
    tz.from_local_datetime(&d.and_hms_opt(h, m, 0).unwrap()).unwrap().to_rfc3339_opts(chrono::SecondsFormat::Secs, false)
}

fn timed(monday: NaiveDate, day: i64, s: (u32, u32), e: (u32, u32), title: &str) -> serde_json::Value {
...
async fn main() {
    ...
    let email = args.get(1).expect("usage: seed_fixtures <account_email> [--delete]").clone();
    let delete = args.iter().any(|a| a == "--delete");
    db::init(&config::db_path()).expect("db");
    auth::oauth::init().expect("oauth.json");
    let account = db::call(move |c| Ok(db::queries::accounts::list_accounts(c)?.into_iter().find(|a| a.email.as_deref() == Some(&email))))
        .await
        .expect("db")
    ...
    let page = client.calendar_list_page(&token, None, None).await.expect("calendarList");
    let existing = page.items.iter().find(|c| c.summary.as_deref() == Some(NAME)).map(|c| c.id.clone());
    if delete {
        match existing {
            Some(id) => {
    ...
    let cal_id = match existing {
        ...
        None => {
            let created = client.calendar_insert(&token, NAME, TZ).await.expect("calendars.insert");
            let id = created["id"].as_str().expect("id").to_string();
            println!("created {NAME} ({id})");
            id
        }
    };
    let have = client.events_page(&token, &cal_id, None, None).await.expect("events");
    let titles: Vec<String> = have.items.iter().filter_map(|e| e.summary.clone()).collect();

    let today = chrono::Utc::now().with_timezone(&TZ.parse::<chrono_tz::Tz>().unwrap()).date_naive();
    let monday = today - Duration::days(today.weekday().num_days_from_monday() as i64);
    let next_monday = monday + Duration::days(7);
    let self_email = account.email.clone().unwrap();
    let guests: Vec<String> = db::call(|c| Ok(db::queries::accounts::list_accounts(c)?.into_iter().filter(|a| a.kind == "google").filter_map(|a| a.email).collect()))
        .await
        .unwrap();
    ...
    let mut attendees = vec![json!({ "email": self_email, "organizer": true, "responseStatus": "accepted" })];
    if let Some(g) = &other_guest {
        attendees.push(json!({ "email": g, "responseStatus": "accepted" }));
    }
    attendees.push(json!({ "email": "guest.pending@example.com", "responseStatus": "needsAction" }));
    ...
    tentative["attendees"] = json!([{ "email": self_email, "self": true, "responseStatus": "tentative" }]);
    events.push(tentative);
    let mut declined = timed(monday, 4, (14, 0), (15, 0), "Declined");
    declined["attendees"] = json!([{ "email": self_email, "self": true, "responseStatus": "declined" }]);
    events.push(declined);
    for i in 0..5u32 {
        events.push(timed(next_monday, 0, (8 + i * 1, 0), (8 + i * 1, 30), &format!("Month {}", i + 1)));
    }
    ...
    for ev in events {
        ...
        match client.event_insert(&token, &cal_id, &body, SendUpdates::None).await {
