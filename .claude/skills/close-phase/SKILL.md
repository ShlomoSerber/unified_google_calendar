...
3. If the phase touched UI or sync (phases 2 to 7), ask the user to start the app in week view with their accounts, then run `bash scripts/measure-ram.sh` and paste the table into `docs/99-decisiones.md` under "Mediciones de RAM por fase". If total PSS exceeds 150 MB, apply the measures of `docs/02-arquitectura.md` section 6 in order before closing.
