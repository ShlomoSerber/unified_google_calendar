...
// Usage: node scripts/measure/capture.mjs [component ...]   (default: all)
...
const WEEK_PAST = 'https://calendar.google.com/calendar/u/0/r/week/2026/9/7';
...
export const COMPONENTS = {
  ...
  create_button: {
    ...
    states: {
      open: { prepare: async (cdp) => { await cdp.clickElement(`[...document.querySelectorAll('button')].find(b => /Create/.test(b.innerText||'') && b.getBoundingClientRect().x < 200)`); await cdp.sleep(1200); }, root: `[...document.querySelectorAll('[role=menu]')].find(m => m.getBoundingClientRect().width > 0)` },
    },
  },
  mini_calendar: {
    url: WEEK,
  ...
  hour_grid: { url: WEEK, root: `document.querySelector('[role=main] [role=grid]')` },
  now_line: {
    url: WEEK,
...
COMPONENTS.event_chip.states.tentative = {
  ...
  root: `[...document.querySelectorAll('[role=main] [role=button]')].filter(b => /, Tentative, /.test((b.innerText||'').replace(/\\n/g,' '))).pop()`,
  cleanup: async (cdp) => { await toggleCalendar(cdp, 'Shlomo Serber', false); },
};
COMPONENTS.event_chip.states.declined = {
  prepare: async (cdp) => { await toggleCalendar(cdp, 'Shlomo Serber', true); },
  root: `[...document.querySelectorAll('[role=main] [role=button]')].filter(b => /, Declined, /.test((b.innerText||'').replace(/\\n/g,' '))).pop()`,
  cleanup: async (cdp) => { await toggleCalendar(cdp, 'Shlomo Serber', false); },
};
  ...
  try {
    ...
    for (const theme of themes) {
      ...
      for (const name of names) {
        ...
        const variants = [{ key: '', ...comp }, ...Object.entries(comp.states || {}).map(([k, v]) => ({ key: k, ...v, url: v.url || comp.url }))];
        for (const v of variants) {
          ...
          try {
            ...
            try { if (v.cleanup) await v.cleanup(cdp); } catch {}
          }
        }
      }
    }
